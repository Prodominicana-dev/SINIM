 ...
 useEffect(() => {
  ...
  if (productSearch) {
    ...
    filteredRamis = filteredRamis.filter((rami) => {
      ...
      return productName.includes(productSearchLower) || productCode.includes(productSearchLower);
    });
  }
    ...
    <>
     ...
     permissionsList={["create:ramis", "update:ramis", "delete:ramis"]}>
      ...
      <div className="flex flex-col w-full p-4 space-y-2 sm:p-8 sm:space-y-0 sm:flex-row">
        <div className="flex items-center justify-start w-full text-xl font-semibold text-center text-black sm:text-left sm:w-4/12">Cantidad de RAMI: {total}</div>
      ...
      <div className={`${filterVisible} flex flex-row justify-end w-full px-4 space-x-5 sm:px-8`}>
      <input
            type="text"
