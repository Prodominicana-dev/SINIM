...
import { Avatar, Input, Option, Select, Spinner, Typography } from '@material-tailwind/react'
...
export default function page() {
    ...
    const [country, setCountry] = useState({ countryCode: 'DO', countryCallingCode: '+1'});
    const [myCountryCodesArray, setMyCountryCodesArray] = useState(() => {
       ...
       return list.sort((a, b) => a.countryCallingCode.localeCompare(b.countryCallingCode));
    });
          ...
          const getUserData = async () => {
           const res = await axios.get(url, {
            ...
            if(res.data.user_metadata){
                setAddress(res.data.user_metadata.address ? res.data.user_metadata.address : '')
                setDocumentation(res.data.user_metadata.documentation.documentation ? res.data.user_metadata.documentation.documentation : '')
                setTypeDocumentation(res.data.user_metadata.documentation.typeDocumentation ? res.data.user_metadata.documentation.typeDocumentation : '')
                setName(res.data.user_metadata.given_name ? res.data.user_metadata.given_name : '')
                setFullName(res.data.user_metadata.given_name && res.data.user_metadata.family_name  ? 
                    `${res.data.user_metadata.given_name} ${res.data.user_metadata.family_name}` : user?.name ? user?.name : '');
                setLastName(res.data.user_metadata.family_name ? res.data.user_metadata.family_name : '')
                setCountry(res.data.user_metadata.phone_number ? res.data.user_metadata.phone_number.country : { countryCode: 'DO', countryCallingCode: '+1'})
                setPhone(res.data.user_metadata.phone_number ? res.data.user_metadata.phone_number.number : '')
            }
    ...
    if(!dataLoaded) return <div className='flex items-center justify-center w-full h-full'><Spinner /></div>
  return (
    <IsLogged>
                ...
                <div className='flex items-center justify-center w-full sm:justify-start sm:w-6/12'>
                    <div className='w-full'>
                        ...
                        <Typography className="text-xs font-thin text-center text-gray-400 sm:text-left sm:text-base">
                             Miembro desde {createdAt ? format(new Date(createdAt), "MMMM yyyy", { locale: es }) : "Fecha no disponible"}
                        </Typography>
                    </div>
                </div>
            </div>
            ...
            <div className='flex flex-col items-center justify-center w-full space-y-4 sm:space-y-10'>
                ...
                <div className='flex flex-col w-full space-y-4 sm:space-y-0 sm:space-x-4 sm:flex-row'>
                    ...
                    <Menu placement="bottom-start">
                        <MenuHandler>
                        <button
                            className="flex flex-row items-center justify-between h-10 gap-2 text-xs font-thin text-black border border-r-0 rounded-l-lg w-36 border-blue-gray-200 bg-blue-gray-500/10"
                        >
                        ...
                        {myCountryCodesArray.map((country, index) => {
                            ...
                            >
                                ...
                                className="object-cover w-5 h-5 rounded-full"
                                />
                                 {country.countryCode} <span className="ml-auto">{country.countryCallingCode}</span>
                            </MenuItem>
                            );
                        })}
                        </MenuList>
                ...
                <div className='flex flex-col w-full space-x-4 sm:flex-row'>
                    <div className="w-full">
                        <Input label="Dirección" type='text' crossOrigin={""} value={address} onChange={(e) => setAddress(e.target.value)}/>
                    </div>
                </div>
                <button className='flex items-center justify-center w-full text-lg font-semibold text-center text-white duration-200 rounded-lg h-14 bg-navy hover:text-white/80 hover:bg-navy/80'
                onClick={handleSubmit}
                disabled={name === '' || lastName === '' || email === '' || phone === '' || address === '' || documentation === '' || typeDocumentation === ''}
                >{ !isSubmitted ? "Guardar" : (<Spinner />) }</button>
            </div>
